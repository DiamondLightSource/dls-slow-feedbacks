...
def test_zero_correction(test_sofb, lattice, mock_caput, mock_caget, caget_responses):
    ...
    numpy.testing.assert_equal(
        lattice.get_element_pv_names("HSTR", "x_kick", pytac.SP), hcm_call[0][0]
    )
    numpy.testing.assert_equal(h_expected, hcm_call[0][1])
    numpy.testing.assert_equal(
        lattice.get_element_pv_names("VSTR", "y_kick", pytac.SP), vcm_call[0][0]
    )
...
def test_correction_fails_if_wrong_dimensions(
    test_sofb, mock_caget, mock_caput, caget_responses
):
...
def test_psc_error_non_zero_multiple(
    test_sofb, mock_caget, mock_caput, caget_responses
):
...
def test_calc_error_reset_after_single(
    test_sofb_server, mock_caget, mock_caput, caget_responses
):
