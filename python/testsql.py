...
def sqlfix():
    ...
    (loc,) = conn.execute("select idx from devices where devices='SR09A-PC-HSTR-01'").fetchone()
    ...
    for n in range(2):
        ...
        conn.execute(query, ("hcm", loc + n, device + ":SETI", device + ":I", 1.8e-5, spos, device))
        device = "SR09S-PC-VSTR-%02d" % (n + 1)
        conn.execute(query, ("vcm", loc + n, device + ":SETI", device + ":I", 1.8e-5, spos, device))
        device = "SR09S-DI-EBPM-%02d" % (n + 1)
        readback = "SR-DI-EBPM-01:SA:X"
        conn.execute(query, ("bpmx", loc + n, readback, readback, 1e-3, spos, device))
        readback = "SR-DI-EBPM-01:SA:Y"
        conn.execute(query, ("bpmy", loc + n, readback, readback, 1e-3, spos, device))
